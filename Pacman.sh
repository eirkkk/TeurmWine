#!/bin/bash
# Termux Prefix Switcher Pacman
# This Program will switch Termux Environment from Pacman
...
BOOTSTRAP_ARM="https://github.com/termux-pacman/termux-packages/releases/download/bootstrap-2023.10.15-r1%2Bpacman-android-7/bootstrap-aarch64.zip?raw=true"

BOOTSTRAP_INTEL="https://github.com/termux-pacman/termux-packages/releases/download/bootstrap-2023.10.15-r1%2Bpacman-android-7/bootstrap-aarch64.zip?raw=true"
...
print-help(){
	...
	Options:
	 help - show this help
	 switch - Switches to Pacman Mode
	 revert - Reverts to 64-bit Mode (This Uninstalls Pacman prefix)

	NOTE: Switching to Pacman will not install packages automatically so you have to install them again if possible
...
switch(){
	...
	case "$(uname -m)" in
		armv8*|armv7*|i*86)
			cat <<- EOM
			Cannot Switch Environment: Your device is running in pacman
			EOM
			exit 2
			;;
		*) ;;
	...
	echo "[*] Downloading Pacman termux bootstrap"
	case "$(uname -m)" in
		aarch64)
	...
	echo "[*] Unpacking Pacman termux bootstrap"
	sleep 3
	chmod 755 usr -R ||:
...
revert(){
	...
	echo "[*] Purging Pacman environment"
	...
	# Kill Current Process and Purge Pacman prefix
